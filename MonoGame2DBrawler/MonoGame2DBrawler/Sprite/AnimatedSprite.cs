...
namespace MonoGame2DBrawler.Sprite
{
    /// <summary>
    ...
    public class AnimatedSprite : Sprite
    {
        ...
        // SpriteBatch and GraphicsDevice are needed to be able to draw the BoundingBox at any time.
        ...
        public AnimatedSprite(string name, Vector2 position, PlayerIndex playerIndex, GraphicsDevice graphicsDevice, SpriteBatch spriteBatch, Texture2D texture = null, KeyboardInput keyboardInput = null, GamePadInput gamePadInput = null) : base(name, position, playerIndex, texture, keyboardInput, gamePadInput)
        {
        ...
        public void Draw(List<AnimatedSprite> animSprites = null)
        {
            _spriteBatch.Draw(_texture, _position + _offsets[_currentFrame], _frames[_currentFrame][_frameIndex], Color.White);
            ...
            if (animSprites != null)
            {
                foreach (AnimatedSprite a in animSprites)
                {
                    if (this.CollidesWith(a))
                    {
                        ...
                        a.DrawBoundingBox(Color.Red);
                    }
                    else
                    {
                        DrawBoundingBox(Color.Blue);
                        a.DrawBoundingBox(Color.Blue);
                    }
                }
            }
            else
        ...
        public void DrawBoundingBox(Color borderColor)
        {
            if (!_drawBoundingBox)
                return;

            _boundingBoxTexture = new Texture2D(_graphicsDevice, 1, 1, false, SurfaceFormat.Color);
            _boundingBoxTexture.SetData(new[] { Color.White });

            // Draw top line
            _spriteBatch.Draw(_boundingBoxTexture,
                new Rectangle(_boundingBox.X, _boundingBox.Y, _frames[_currentFrame][_frameIndex].Width, 2),
                borderColor);

            // Draw left line
            _spriteBatch.Draw(_boundingBoxTexture,
                new Rectangle(_boundingBox.X, _boundingBox.Y, 2, _frames[_currentFrame][_frameIndex].Height), borderColor);

            // Draw right line
            _spriteBatch.Draw(_boundingBoxTexture,
                new Rectangle(_boundingBox.X + _frames[_currentFrame][_frameIndex].Width - 2, _boundingBox.Y, 2,
                _frames[_currentFrame][_frameIndex].Height), borderColor);

            // Draw bottom line
            _spriteBatch.Draw(_boundingBoxTexture,
                new Rectangle(_boundingBox.X,
                _boundingBox.Y + _frames[_currentFrame][_frameIndex].Height - 2,
                _frames[_currentFrame][_frameIndex].Width, 2), borderColor);
        }
        ...
        public void PlayAnimation(string name)
        {
            ...
            if (_currentFrame != name && _currentDirection.Equals(Direction.none)) // CHANGE == to Equals
            {
                _currentFrame = name;
                _frameIndex = 0;
            }
